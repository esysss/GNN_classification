...
for al in range(10):
    ...
    overal_accuracies.append(np.array(accuracy).mean())

overal_accuracies = np.array(overal_accuracies)
print("#########################################")
print("the overall mean: {} the overall max: {} the overall min: {}".format(np.mean(overal_accuracies),
      np.max(overal_accuracies), np.min(overal_accuracies)))